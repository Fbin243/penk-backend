...
func (ec *executionContext) __resolve_entities(ctx context.Context, representations []map[string]interface{}) []fedruntime.Entity {
	...
	resolveEntity := func(ctx context.Context, typeName string, rep map[string]interface{}, idx []int, i int) (err error) {
		...
		switch typeName {
		case "Fish":
			resolverName, err := entityResolverNameForFish(ctx, rep)
			if err != nil {
				return fmt.Errorf(`finding resolver for Entity "Fish": %w`, err)
			}
			switch resolverName {

			case "findFishByProfileID":
				id0, err := ec.unmarshalNObjectID2goᚗmongodbᚗorgᚋmongoᚑdriverᚋbsonᚋprimitiveᚐObjectID(ctx, rep["profileID"])
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findFishByProfileID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindFishByProfileID(ctx, id0)
				if err != nil {
					return fmt.Errorf(`resolving Entity "Fish": %w`, err)
				}
...
func entityResolverNameForFish(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
		...
		return "findFishByProfileID", nil
	}
	return "", fmt.Errorf("%w for Fish", ErrTypeNotFound)
}
